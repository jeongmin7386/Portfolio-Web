...
type UserProjectDetailPageProps = {
  params: Promise<{
    ownerKey: string;
    slug: string;
  }>;
...
export async function generateMetadata({
  params
}: UserProjectDetailPageProps): Promise<Metadata> {
  const { ownerKey: rawOwnerKey, slug } = await params;
  const ownerKey = normalizeContentOwnerKey(rawOwnerKey);
  const project = await getProjectBySlug(slug, ownerKey);
...
export default async function UserProjectDetailPage({
  ...
  const [project, projects] = await Promise.all([
    getProjectBySlug(slug, ownerKey),
