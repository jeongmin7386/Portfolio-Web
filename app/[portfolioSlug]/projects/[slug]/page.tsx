...
type PublicPortfolioProjectDetailPageProps = {
  params: Promise<{
...
export async function generateMetadata({
  params
}: PublicPortfolioProjectDetailPageProps): Promise<Metadata> {
  const { portfolioSlug, slug } = await params;
  ...
  const project = await getProjectBySlug(slug, portfolio.ownerKey);
...
export default async function PublicPortfolioProjectDetailPage({
  params
}: PublicPortfolioProjectDetailPageProps) {
  const { portfolioSlug, slug } = await params;
  const portfolio = await getPublishedPortfolioByPublicSlug(portfolioSlug);
  ...
  const [project, projects] = await Promise.all([
    getProjectBySlug(slug, portfolio.ownerKey),
